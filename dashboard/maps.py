   ...
   clusters: None|dict[int,str]=None,
    ...
    for _, (ncessch_id, *_) in gdf.iterrows():
        ...
        try:
           ...
           print(f"warning: could not find {s_id} in {focal_district_id} interdistrict sim")
...
def school_colors_v0(schools: tuple[int, ...]) -> dict[int, str]:
   ...
   """
   if len(schools) <= len(_palette): return dict(zip(schools, _palette))
   adj = {}
   seen = set()
   for school in schools:
      district_id = SCHOOL_ID_TO_DISTRICT_ID[school]
      if district_id in seen: continue
   ...
   random.seed(23)
   for palette in possible_colors.values(): random.shuffle(palette)
   sorted_schools = sorted(schools, key=lambda s: len(adj.get(f"{s:012d}", ())), reverse=True)
   colors = {}
...
def render_map(existing=None, rendered=None, *, height=MAP_HEIGHT_PIXELS):
   if rendered is None:
      ...
      if existing is not None: figure.add_child(existing)
...
def _demographics_tooltip(population_before: Population, population_after: None|Population=None) -> str:
   """HTML was a mistake
   """
   if population_before == population_after: return _demographics_tooltip(population_before)
   ...
   if population_after is None:
      tooltips.append(f'''<tr style='border-bottom: 1px solid black;'><th>&nbsp;</th><th style='{_center}'>Count</th></tr>''')
      ...
      tooltips.append(f"<tr style='border-bottom: 1px solid black;'><th>&nbsp;</th><th style='{_center}'>Before</th><th style='{_center}'>After</th></tr>")
      for demo, demo_name in DEMOGRAPHICS.items():
         pb = round(population_before[demo])
         pa = round(population_after[demo])
         if demo != "total":
            tooltip = f"<tr><td style='{_right}'>{demo_name}</td><td style='{_center}'>"
...
def make_a_layer(
   ...
   *,
   ...
   gdf = _read_geodata(district.state, district.nces_id, neighboring_district_ids, clusters).copy()
   # add the tooltips
   tooltips = []
   if cluster_them_up:  # cluster-based tooltips
      for cluster_name in gdf["cluster"].astype(str):
         ...
         for school in cluster:
            ...
            if school.district_id != district.nces_id:
               district_name = DISTRICTS_IN_STATE[district.state].get(school.district_id, f"NCES ID: {school.district_id:07d}")
               school_name += f"<br>&emsp;&emsp;(from neighboring district, {district_name})"
      ...
      for s_id in gdf["ncessch"].astype(int):
         ...
         if school.district_id != district.nces_id:
            district_name = DISTRICTS_IN_STATE[district.state].get(school.district_id, f"NCES ID: {school.district_id:07d}")
            school_name += f"<br>&emsp;&emsp;(from neighboring district, {district_name})"
         demographics_info = _demographics_tooltip(school.population_before, school.population_after)
   ...
   match layer_kind:
      ...
      case MapLayer.DEMOGRAPHICS_AFTER:
         colors = school_colors_demographics_v0(district, clusters, demo=demo, after=True)
...
def make_outline_layer(nces_ids: Collection[int], color: str|None=None) -> folium.FeatureGroup|folium.GeoJson:
   ...
   for nces_id in nces_ids:
      ...
      path = Path("data/school_attendance_boundaries/outlines") / f"{state}/{district_id_str}.csv"
...
def make_school_markers_layer(
   ...
   *,
   ...
   _style = lambda alignment: f"font-size: small; padding: 0.2ch 0.6ch; text-align: {alignment};"
   _right = _style("right")
   _center = _style("center")
   _left = _style("left")
   for s_id, school in district.schools_in_simulation.items():
      if mark_these_schools is not None:
         if (s_id not in mark_these_schools) ^ except_: continue
      if school.location is None: continue
      if school.grade_span_before != school.grade_span_after:
         ...
         grade_span_before = _describe_grade_span(school.grade_span_before, short=True)
...
def draw(where, district: District, *, focal_demo="color", cluster: None|Collection[School]=None):
        ...
        try:
            with spinner_placeholder:
                with st.spinner(f"Drawing the map..."):
                    ...
                    with map_placeholder: render_map(base_map)
                    ...
                    before_layer = make_a_layer(district, MapLayer.CATCHMENT_AREAS_BEFORE, these)
                    before_layer.add_to(base_map)
                    demographics_layer = make_a_layer(district, MapLayer.DEMOGRAPHICS_BEFORE, these)
                    demographics_layer.add_to(base_map)
                    if focal_demo != "color":
                        extra_layer = make_a_layer(district, MapLayer.DEMOGRAPHICS_BEFORE, these, demo=focal_demo)
                        extra_layer.add_to(base_map)

                    clusters_layer = make_a_layer(district, MapLayer.CATCHMENT_AREAS_AFTER, these)
                    clusters_layer.add_to(base_map)
                    clustered_demographics_layer = make_a_layer(district, MapLayer.DEMOGRAPHICS_AFTER, these, is_default=True)
                    clustered_demographics_layer.add_to(base_map)
                    if focal_demo != "color":
                        clustered_extra_layer = make_a_layer(district, MapLayer.DEMOGRAPHICS_AFTER, these, demo=focal_demo)
                        clustered_extra_layer.add_to(base_map)

                    # district outlines
                    district_ids = {district.nces_id} | {s.district_id for s in district.schools_from_neighboring_districts.values()}
                    ...
                    school_markers = make_school_markers_layer(district, these or None, except_=True)
                    school_markers.add_to(base_map)
