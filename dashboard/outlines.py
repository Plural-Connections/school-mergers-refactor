...
def make_all_the_outlines(*,
   ...
   for folder in intake.iterdir():
      ...
      if not folder.name.isupper() or not len(folder.name) == 2: continue
   ...
   for folder in all_states:
      ...
      for csv in folder.iterdir():
         ...
         gdf_out = pd.concat([gdf_out, gpd.GeoDataFrame({"nces_id": [f"{district_id:07d}"], "geometry": [entire_district]})])
         # outline of each school
         for _, (school_id, geom, _) in gdf.iterrows():
            school_id = int(school_id)
            geom = close_holes(geom)
            gdf_out = pd.concat([gdf_out, gpd.GeoDataFrame({"nces_id": [f"{school_id:012d}"], "geometry": [geom]})])
