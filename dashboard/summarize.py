...
def _describe_grade_span(grade_span: list[str], *, short: bool=False) -> str:
   if len(grade_span) == 0: return "no grades"
   if len(grade_span) == 1:
      grade = grade_span[0]
      return (
   ...
   if len(grade_span) == 2: connector = ", " if short else " and "
   return (
      f"{lookup.get(lower, lower)}{connector}"
      f"{lookup.get(upper, upper)}"
   )

def md(where, text: str, **kwargs):
   """Pre-process markdown
   """
   text = text.replace("<|", '''<mark style="background-color: rgb(253, 222, 150);">''')
   text = text.replace("|>", "</mark>")
   where.markdown(text, unsafe_allow_html=True, **kwargs)

def _html_format(text: str, *,
   bold: bool=False,
   italic: bool=False,
   ...
   """
   style = {}
   if font_size: style["font-size"] = str(font_size)
   if color: style["color"] = str(color)
   style_str = " ".join(f"{k}: {v};" for k, v in style.items())
   if code: text = f"<code>{text}</code>"
...
def demos_grades(where, cluster: list[School], *, when: str="after", use_rainbow_colors=True):
...
def demographics_v0(where, cluster: list[School], *, when: str="after", use_rainbow_colors=True):
   ...
   if use_rainbow_colors:
      colors = alt.Color(
         "Demographic:N",
         scale=alt.Scale(
            domain=DEMOGRAPHIC_LABELS,
            range=rainbow_colors(len(DEMOGRAPHICS))
         ),
      )

   chart = alt.Chart(df_demos_plot).mark_bar().encode(
...
def _determine_majority_demographics(population: Population, threshold: float=0.4) -> list[str]:
...
def _description_district_demographics(district: District) -> str:
   ...
   n_schools_ = (
      f"<|{n_schools}|> closed-enrollment elementary school"
...
def district_demographics_v0(where, district: District, *, use_rainbow_colors=True):
   ...
   if use_rainbow_colors:
      colors = alt.Color(
         ...
         scale=alt.Scale(
            domain=DEMOGRAPHIC_LABELS,
            range=rainbow_colors(len(DEMOGRAPHIC_LABELS))
         ),
      )
...
def _describe_minutes(minutes: float, *, handwavy=False) -> str:
   if math.isnan(minutes): return "[NaN]"
   if minutes < 2.25: return "a couple minutes"
   if minutes < 3.25: return "a few minutes"
   if not handwavy: return f"<|{round(minutes)}|> minutes"
   m = math.floor(minutes)
   s = math.ceil(60*(minutes-(m*60)))
   if s < 15: return f"just over <|{m}|> minutes"
   if s > 45: return f"just under <|{m+1}|> minutes"
...
def _format_duration(seconds: int, signed=True) -> str:
   ...
   if h: return f"{sign}{h}h {m:02d}m {s:02d}s"
   if m: return f"{sign}{m}m {s:02d}s"
   return f"{sign}{s}s"
...
def travel_times_v0(where, district: District, *, per_individual=True):
   ...
   if per_individual:
      ...
      md(where, "These travel times are calculated as car driving times. Elementary school students travel to and from school in a variety of ways—by bus, walking, or by car, for instance—so these travel times should interpretted according to local context.")
   else:
      travel_times = analytics.travel_times / 60

   if district.proportion_switched == 0.0: return
   ...
   chart_title = (
      "Travel time per individual across demographics" if per_individual
...
def travel_times_v1(where, analytics: Analytics, *, per_individual=False, use_rainbow_colors=True):
   ...
   if "total" in demographics: demographics.remove("total")
   ...
   chart_title = "Travel times (individual)" if per_individual else "Travel times (total)"
   chart = chart.properties(title=chart_title)
   chart = chart.configure_title(anchor="middle")
   where.altair_chart(chart)

def district(where, district: District):
   num_students = round(sum(
      s.population_before.total or 0
      for s in district.schools.values()
   ))
   data = {
...
def leaflet(where, district: District) -> str:
   ...
   if focal_demo != "color":
      for focal_demo in impact.focal_demos:
         if focal_demo in majority_demos: break
   if focal_demo == "white": focal_demo = "color"
   focal_demo_name = (
      "students of color"
      if focal_demo == "color"
   ...
   determiner = "districts" if district.clusters_from_neighboring_districts else "district"
...
def _make_link(base_url:str, configuration: dict[str, Any], district: District|None=None,
   **kwargs
) -> str:
   ...
   _query_params = {
      ...
      "selected_district": ("nces", lambda district_name: f"{DISTRICTS_IN_STATE[state][district_name]:07d}"),
   }
   query = {}
   for key_name, (new_key_name, func) in _query_params.items():
      if key_name not in configuration: continue
