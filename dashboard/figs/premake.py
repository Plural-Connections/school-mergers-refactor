...
def _all_districts(kind: SimulationKind, interdistrict: bool, *, closed_enrollment_only: bool=True) -> dict[int, District]:
   ...
   for district_id in tqdm(only_these_districts):
      ...
      context = eat.context(state, district_id, interdistrict, _thresholds[kind], folder_name_check=_folder_mapper[kind])
      try:
...
def _predigest_bh_wa():
   ...
   if "pre_dissim_bh_wa" not in df: return
   ...
   df.rename(columns={"pre_dissim_bh_wa": "pre_dissim", "post_dissim_bh_wa": "post_dissim"}, inplace=True)
...
def _generate_top_n_districts(
   ...
   if kind in (SimulationKind.CONSTRAINED_BH_WA,):
      _predigest_bh_wa()
   global ALL_DISTRICTS
   if (kind, interdistrict) not in ALL_DISTRICTS: ALL_DISTRICTS[kind, interdistrict] = _all_districts(kind, interdistrict, closed_enrollment_only=closed_enrollment_only)
   all_districts_sorted = sorted(ALL_DISTRICTS[kind, interdistrict].values(), key=key, reverse=True)
   ...
   data = {
      ...
      "dissim_change": [(d.analytics.post_dissimilarity - d.analytics.pre_dissimilarity)/d.analytics.pre_dissimilarity for d in top_n_districts],
      "pre_times": [d.analytics.travel_times_per_individual.switcher_previous.total/60.0 for d in top_n_districts],
      "post_times": [d.analytics.travel_times_per_individual.switcher_new.total/60.0 for d in top_n_districts],
      "times_change": [
         (d.analytics.travel_times_per_individual.switcher_new.total - d.analytics.travel_times_per_individual.switcher_previous.total) / 60.0
...
def generate_top_n_districts_by_population(kind: SimulationKind,
   ...
   *,
   ...
   _generate_top_n_districts(kind, interdistrict, n, lambda d: d.population.total or 0, output, closed_enrollment_only)
