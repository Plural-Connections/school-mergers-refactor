...
@st.cache_data()
def get_school_changes(kind: SimulationKind, interdistrict: bool=False) -> pd.DataFrame:
   ...
   for district_id in top_200_districts(kind, interdistrict):
      ...
      try:
         ...
         print(f"warning: skipping ({state}:{district_id:07d}) due to AssertionError: {e}")
         continue
      except FileNotFoundError as e:
         print(f"warning: missing simulation? {e}")
         continue
      for cluster in district.clusters.values():
         if not len(cluster) >= 2: continue
         for school in cluster:
            if school.district_id != district_id: continue  # only focal district
            if school.travel_times_previous is None or school.travel_times_new is None: continue
            try:
               before = school.travel_times_previous.total/(60*school.population_before.total)
               after = school.travel_times_new.total/(60*school.population_after.total)
            except ZeroDivisionError:
               continue
            school_info = f"{school.school_name} ({school.ncessch_id:013d}) ({state})"
