...
def eat_context(
    state: str, district_id: int, kind: SimulationKind, interdistrict: bool = False
) -> Simulation:
...
def _get_extra_infos(kind: SimulationKind, interdistrict: bool = False) -> pd.DataFrame:
    if interdistrict:
        filepath = (
            DATA_ROOT
            / "min_num_elem_schools_4_interdistrict"
            / f"consolidated_simulation_results_min_num_elem_schools_4_interdistrict_{_thresholds[kind]}_{interdistrict}.csv"
        )
    else:
        filepath = (
            DATA_ROOT
            / f"min_num_elem_schools_4_{_folder_mapper[kind]}"
            / f"consolidated_simulation_results_min_num_elem_school_4_{_folder_mapper[kind]}_{_thresholds[kind]}_{interdistrict}.csv"
        )
    if kind in (SimulationKind.CONSTRAINED_BH_WA,):
        filepath = (
            DATA_ROOT
            / filepath.parts[-2].replace("elem_schools_", "elem_")
            / filepath.parts[-1].replace("elem_school_", "elem_")
        )
    ...
    for demo in DEMOGRAPHICS:
        ...
        times_df[f"switcher_status_quo_time_num_{demo}"] /= (
            60 * pop_df[f"num_{demo}_switched"]
        )
...
@st.cache_data()
def top_200_districts_df(
    kind: SimulationKind, interdistrict: bool = False, *, extra_info: bool = False
) -> pd.DataFrame:
    ...
    df = pd.read_csv(
        FIGURE_OUTPUT_PATH / f"results_top_200_by_population_{determiner}.csv"
    )
    if extra_info:
