...
def _plot_histogram(df: pd.DataFrame, *,
   color="#8E7AB5",
   ...
   fig = make_subplots(
      cols=1, rows=3 if cumulative else 2,
   ...
   if cumulative:
      ...
      cumsum = np.cumsum(hist); cumsum /= np.max(cumsum)
   ...
   for district_id in top_200_districts(kind):
      ...
      for cluster in district.clusters.values():
         ...
         for school in cluster:
            ...
            assert before is not None and not isinf(before) and not isnan(before), f"{before=!r}"
            assert after is not None and not isinf(after) and not isnan(after), f"{after=!r}"
...
def plot_changes(kind: SimulationKind, *,
   show: bool=False,
   save: bool=True,
   ...
   if save: fig.write_image(output)
